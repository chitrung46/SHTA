...
class SHTA(nn.Module):
    def __init__(self, args, device, dim_in):
        ...
        self.layers = nn.ModuleList(
            [
                SHTBlock(device=device, input_dim=self.channels, dynamic=self.dynamic, num_nodes=self.num_nodes, cuts=12,
                      cut_size=6, no_proxies=2, memory_size=self.memory_size),
                SHTBlock(device=device, input_dim=self.channels, dynamic=self.dynamic, num_nodes=self.num_nodes, cuts=3,
                      cut_size=4, no_proxies=2, memory_size=self.memory_size),
                SHTBlock(device=device, input_dim=self.channels, dynamic=self.dynamic, num_nodes=self.num_nodes, cuts=1,
...
class SHTBlock(nn.Module):
    def __init__(self, device, input_dim, num_nodes, cuts, cut_size, dynamic, memory_size, no_proxies):
        ...
        self.temporal_att = HTSA(input_dim, num_nodes=num_nodes, cut_size=cut_size)
